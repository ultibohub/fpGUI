...
//Ultibo To Do //Porting of this unit is incomplete. Quite a bit of what is here
               //will work without modification. What is missing is the draw()
               //function which takes a handle and draws the pixel map (bitmap)
               //onto the device represented by the handle.
               //
               //This could use the ConsoleGraphics unit to draw onto a standard
               //console window (See agg_platform_support.pas)
...
type
 pixel_map = object
   ...
   procedure draw(h_dc : THandle; device_rect : PRect = NIL; bmp_rect : PRect = NIL ); overload;
   procedure draw(h_dc : THandle; x ,y : int; scale : double = 1.0 ); overload;
...
{ DRAW }
procedure pixel_map.draw(h_dc : THandle; device_rect : PRect = NIL; bmp_rect : PRect = NIL );
var
 bmp_x ,
 bmp_y ,
...
begin
 ...
 //Ultibo To Do //Bitmap drawing support
 (*bmp_x:=0;
 bmp_y:=0;
...
{ DRAW }
procedure pixel_map.draw(h_dc : THandle; x ,y : int; scale : double = 1.0 );
var
 rect : TRect;
