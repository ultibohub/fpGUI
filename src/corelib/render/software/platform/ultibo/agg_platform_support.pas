...
//Ultibo To Do //Porting of this unit is incomplete. Quite a bit of what is here
               //(and in agg_ultibo_bmp.pas) will work without modification. What
               //is missing right now is creating some form of window to draw the
               //pixels on and then drawing the rendered image to that window when
               //the demo asks us to.
               //
               //This could be a good usage of the new ConsoleGraphics unit to allow
               //drawing images alongside text on a standard console window.
...
// our agg demos, it's okay to do that this way. See {hack}.
 m_sw_freq  ,
 m_sw_start : int64; //Ultibo To Do //This hack probably doesn't apply to Ultibo
...
type
 ...
 platform_specific = object
   ...
   m_current_dc  : THandle;

   //m_sw_freq  ,
   //m_sw_start : int64;{hack} //Ultibo To Do //This hack probably doesn't apply to Ultibo see above
   ...
   procedure display_pmap(dc : THandle; src : rendering_buffer_ptr ); {OpenCPL++} virtual; {/OpenCPL++}
...
begin
 ...
 //Ultibo To Do //Add keyboard support (Use KEY_* constants in GlobalConst)
 ...
 //QueryPerformanceFrequency(m_sw_freq );{hack} //Ultibo To Do //Use CLOCK_FREQUENCY ?
 //QueryPerformanceCounter  (m_sw_start ); //Ultibo To Do //Use Platform.ClockGetTotal ?
...
begin
 ...
 // SetWindowText(m_specific.m_hwnd ,@m_caption[1 ] ); //Ultibo To Do
...
{ window_proc }
function window_proc(Wnd : THandle; Msg : UINT; WPar : WParam; LPar : LParam ) : LResult; stdcall;
(*var
 ps  : TPaintStruct;
...
begin
 //Ultibo To Do //This is probably not required although the functionality will be reproduced elsewhere
...
begin
 ...
 //Ultibo To Do //Create a window and a pixel map, setup internal parameters etc
...
begin
 repeat
  if m_wait_mode then
   begin
    //Ultibo To Do //Polling/Waiting for Mouse/Keyboard messages etc
...
begin
 //Ultibo To Do //Destroy the window and clean up
...
begin
 ...
 //InvalidateRect(m_specific.m_hwnd ,0 ,false ); //Ultibo To Do 
...
begin
 ...
 //m_specific.display_pmap(dc ,@m_rbuf_window ); //Ultibo To Do //Draw the render buffer to the screen //See display_pmap
...
begin
 //MessageBox(m_specific.m_hwnd ,@msg[0 ] ,'AGG Message' ,MB_OK ); //Ultibo To Do //Not sure about message boxes yet
...
begin
 //QueryPerformanceCounter({m_specific.}m_sw_start );{hack} //Ultibo To Do //Use Platform.ClockGetTotal ?
...
var
 stop : Int64; //TLargeInteger;

begin
 //QueryPerformanceCounter(stop ); //Ultibo To Do //Use Platform.ClockGetTotal ?
