...
{Ultibo version of the fpgCanvas example

 Compile using the build_ultibo.bat file in this folder
 
}

program fpgcanvas_ultibo;
...
//  Canvas.DrawLine(1,y, 10, y);
  ...
  //Canvas.DrawString(5, y, 'Russian (UTF-8) text -> 񌖘'); //Ultibo To Do //Needs FreeType
  y := y + Canvas.Font.Height;
  fnt := fpgApplication.GetFont('verdana14_bold'); //Ultibo To Do //Need to account for FreeType
  Canvas.Font := fnt;
...
begin
  try 
   Sleep(10000);
   LoggingOutput('Starting fpGUI CanvasTest'); 
